...
\begin{enumerate}
\item Specific Examples: \ilds{ \diff{}{x} x^n } or \ilds{ \diff{}{x} \left( \frac{1}{x} \right) }
...
So we know the value of \ilds{ \diff{}{x} \sin x } and \ilds{ \diff{}{x} \cos x } at $x = 0$. Let us find for arbitary $x$. 
...
Since \ilds{ \frac{\cos \Delta x - 1}{\Delta x} \to 0 } and that \ilds{ \frac{\sin \Delta x}{\Delta x} \to 1 }, the above equation simplifies to $$\diff{}{x} \sin x = \cos x$$
...
\subsection{Derivative of \ilds{ \frac{1}{x} }}
Taking the limit
\begin{align*}
	\diff{}{x} \frac{1}{x} 
