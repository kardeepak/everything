...
\item Define $\alpha + \beta = \{ r + s | r \in \alpha, s \in \beta\}$.

Let us check that it meets the requirements of \ref{add} (A).

\begin{enumerate}[(\text{A}1)]
    \item We need to prove that $\alpha + \beta$ is a cut of $Q$.
    Since $\alpha$ is not empty and $\beta$ is not empty so $\alpha + \beta$ is not empty.
    Also since $\alpha \neq Q$ and $\beta \neq Q$
\end{enumerate}
